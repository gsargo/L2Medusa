...
public class old_PvPFlagZone_SiegeLike extends ZoneType
{
	...
	List<Creature> _players = new CopyOnWriteArrayList<>();
	
	public old_PvPFlagZone_SiegeLike(int id)
	{
		super(id);
	...
	protected void onEnter(Creature character)
	{
		...
		if (player != null)
		{
			...
			if (_pvp)
			{
				...
				if (_pvpTask == null)
					_pvpTask = ThreadPool.scheduleAtFixedRate(() -> _players.forEach(V -> PvpFlagTaskManager.getInstance().add(V.getActingPlayer(), System.currentTimeMillis() + _pvpTime)), 0, _pvpTime);
			}

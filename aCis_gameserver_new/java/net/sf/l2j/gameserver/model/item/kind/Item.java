...
public abstract class Item
{
	...
	
	{
		...
		SLOTS.put("lonly_ear", SLOT_L_EAR); // left ear restriction
		SLOTS.put("ronly_ear", SLOT_R_EAR); // right ear restriction
		SLOTS.put("rfinger;lfinger", SLOT_R_FINGER | SLOT_L_FINGER);
		SLOTS.put("lonly_finger", SLOT_L_FINGER); // left finger restriction
		SLOTS.put("ronly_finger", SLOT_R_FINGER); // right finger reistriction
		SLOTS.put("none", SLOT_NONE);
		SLOTS.put("none_shadow", SLOT_NONE_SHADOW);// skins_shadow
	...
	public static final int SLOT_NONE_SHADOW = 0x0000;
	...
	// public static final int SLOT_RONLY_FINGER = 0x0010; // restrict rfinger
	
	public static final int SLOT_R_FINGER = 0x0010;
	//public static final int SLOT_R_FINGER = 0x04;
	//public static final int SLOT_L_FINGER = 0x05;
	...
	private final boolean _isBowRestricted;
	private final boolean _isNobleRestricted;
	private final boolean _isDaggerRestricted;
	private final boolean _isTankOnlyRestricted;
	private final boolean _isLightRestricted;
	private final boolean _isHeavyRestricted;
	private final boolean _isFighterRestricted;
	
	private final boolean _hasCastle;
	private final boolean _hasClanhall;
	...
	private final boolean _tepmlate;
	private final boolean _tepmlate_hair;
	
	private int _hairTepmlate;
	private int _faceTepmlate;
	private int _chestTepmlate;
	private int _legsTepmlate;
	private int _glovesTepmlate;
	private int _feetTepmlate;
	...
	protected Item(StatSet set)
	{
		...
		_isBowRestricted = set.getBool("is_bow_restricted", false);
		_isNobleRestricted = set.getBool("is_noble_restricted", false);
		_isDaggerRestricted = set.getBool("is_dagger_restricted", false);
		_isTankOnlyRestricted = set.getBool("is_tankonly_restricted", false);
		_isLightRestricted = set.getBool("is_light_restricted", false);
		_isHeavyRestricted = set.getBool("is_heavy_restricted", false);
		_isFighterRestricted = set.getBool("is_fighter_restricted", false);
		
		_hasClanhall = set.getBool("has_clanhall", false);
		_hasCastle = set.getBool("has_castle", false);
		...
		_tepmlate = set.getBool("template", false);
		if (_tepmlate)
		{
			_hairTepmlate = set.getInteger("temp_hair", 0);
			_faceTepmlate = set.getInteger("temp_face", 0);
			_chestTepmlate = set.getInteger("temp_chest", 0);
			_legsTepmlate = set.getInteger("temp_legs", 0);
			_glovesTepmlate = set.getInteger("temp_gloves", 0);
			_feetTepmlate = set.getInteger("temp_feet", 0);
		}
		
		_tepmlate_hair = set.getBool("template_hair", false);
		if (_tepmlate_hair)
		{
			_hairTepmlate = set.getInteger("temp_hair", 0);
		}
	...
	public boolean checkCondition(Creature creature, WorldObject object, boolean sendMessage)
	{
		...
		if (isNobleRestrictedItem() && creature instanceof Player && !creature.getActingPlayer().isNoble())
		{
			creature.getActingPlayer().sendMessage("Usage of this item is reserved for users on noblesse status.");
			return false;
		}
		
		else if (hasCastleRestrictedItem() && creature instanceof Player && (creature.getActingPlayer().getClan() == null || !creature.getActingPlayer().getClan().hasCastle()))
		{
			creature.getActingPlayer().sendMessage("Usage of this item is reserved for castle owners.");
			return false;
		}
		
		else if (hasClanhallRestrictedItem() && creature instanceof Player && (creature.getActingPlayer().getClan() == null || !creature.getActingPlayer().getClan().hasClanHall()))
		{
			creature.getActingPlayer().sendMessage("Usage of this item is reserved for clan hall owners.");
			return false;
		}
		
		else if (isDaggerRestrictedItem() && creature instanceof Player && (creature.getActingPlayer().getClassId().isDagger() ))
		{
		...
		/*else if (isTankOnlyItem() && creature.getActingPlayer().getClassId().isHealer() || creature.getActingPlayer().getClassId().isDagger())
		{
			creature.getActingPlayer().sendMessage("The use of that item is prohibited for your class.");
			return false;
		}*/
		/*else if (isBowRestrictedItem() && !(creature.getActingPlayer().getClassId().isArcher() || creature.getActingPlayer().getClassId().isHealer() || creature.getActingPlayer().getClassId().isSummoner()))
		{
			creature.getActingPlayer().sendMessage("The use of bow is prohibited for your class.");
			return false;
		}	
		
		else if (isLightRestrictedItem() && !(creature.getActingPlayer().getClassId().CanUseLight()))
		{
			creature.getActingPlayer().sendMessage("The use of that item is prohibited for your class.");
			return false;
		}
		else if (isHeavyRestrictedItem() && !(creature.getActingPlayer().getClassId().CanUseHeavy()))
		{
			creature.getActingPlayer().sendMessage("The use of that item is prohibited for your class.");
			return false;
		}
	...
	public boolean isBowRestrictedItem()
	{
		return _isBowRestricted;
	}
	
	public boolean isNobleRestrictedItem()
	{
		return _isNobleRestricted;
	}
	
	public boolean isDaggerRestrictedItem()
	{
		return _isDaggerRestricted;
	}
	
	public boolean isTankOnlyItem()
	{
		return _isTankOnlyRestricted;
	}
	
	public boolean isLightRestrictedItem()
	{
		return _isLightRestricted;
	}
	
	public boolean isHeavyRestrictedItem()
	{
		return _isHeavyRestricted;
	}
	
	public boolean isFighterRestrictedItem()
	{
		return _isFighterRestricted;
	}
	
	public boolean hasCastleRestrictedItem()
	{
		return _hasCastle;
	}
	
	public boolean hasClanhallRestrictedItem()
	{
		return _hasClanhall;
	}

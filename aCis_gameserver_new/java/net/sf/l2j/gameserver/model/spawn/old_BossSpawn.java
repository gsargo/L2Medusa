 ...
 */
public class old_BossSpawn
{
	protected static final CLogger LOGGER = new CLogger(old_BossSpawn.class.getName());
	 ...
	 * <li>Refresh this {@link old_BossSpawn} holder.</li>
	...
	public void onDeath()
	{
		...
		{
			respawnDelay = loadTime(Config.RAID_BOSS_RESPAWN_TIME.get(_spawn.getNpcId()).split("~")) - System.currentTimeMillis();
	...
	long loadTime(String[] _data)
	{
		...
		try
		{
			...
			for (String timeOfDay : _data)
			{
				...
				testStartTime.set(Calendar.HOUR_OF_DAY, Integer.parseInt(splitTimeOfDay[1]));
				testStartTime.set(Calendar.MINUTE, Integer.parseInt(splitTimeOfDay[2]));
	 ...
	 * <li>Save informations on database.</li>
	 * <li>Delete the reference of Spawn from this {@link old_BossSpawn} holder.</li>
	 * </ul>
	 */
	...
	/**
	 * Update the respawn time and current HP/MP of this {@link old_BossSpawn} on the database.
	 */

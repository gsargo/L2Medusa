...
public class PlayerStatus extends PlayableStatus<Player>
{
	...
	public final void reduceHp(double value, Creature attacker, boolean awake, boolean isDOT, boolean isHPConsumption, boolean ignoreCP)
	{
		...
		if (attacker != null && attacker != _actor)
		{
			...
			{
				if(_actor.isInsidePvPZone() && !_actor.isInsideClanwarZone())
					_actor.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.INCOGNITO_GAVE_YOU_S1_DMG).addNumber(fullValue));
				else
		...
		/*if (_actor.getTempChest() != 0 || _actor.getTempFace() != 0 || _actor.getTempFeet() != 0 || _actor.getTempHair() != 0 || _actor.getTempGloves() != 0 || _actor.getTempLegs() != 0)
		{
			...
			{
				_actor.setTempHair(0);
				_actor.setTempFace(0);
				_actor.setTempChest(0);
				_actor.setTempLegs(0);
				_actor.setTempGloves(0);
				_actor.setTempFeet(0);
				_actor._costumeInterval = System.currentTimeMillis() + _actor._costumeIntervalTime;
	 ...
	 * @return True if this {@link Player} exceeded the authorized inventory slots ratio (which is 85%), or false otherwise.
	 */
	public boolean isOverburden()
	{
		return (double) _actor.getInventory().getSize() / getInventoryLimit() >= 0.85;
	}

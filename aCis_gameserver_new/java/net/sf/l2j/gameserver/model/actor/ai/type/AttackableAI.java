...
public class AttackableAI extends CreatureAI implements Runnable
{
	...
	protected void thinkAttack()
	{
		...
		if (getActor() != null && getActor().getTarget() != null && getActor().getSpawn() != null)
		{
			SpawnLocation loc = getActor().getSpawn().getLoc();		
			//getActor().getNpcId()) != 60036
			if ((exclude_from_range(getActor().getNpcId()))  && !MathUtil.checkIfInRange(Config.RAID_BOSS_RANGE, loc, getActor().getPosition(), true))
				getActor().teleportTo(loc.getX(), loc.getY(), loc.getZ(), 50);
		}
		super.thinkAttack();
	}
	//Exclude specific Monsters/Bosses from the RAID_BOSS_RANGE (not to teleport back to spawn)
	private static boolean exclude_from_range(int npc_id)
	{
		if(npc_id == 60036 || npc_id == 62326 || npc_id == 60050) // Hades , PK KILLER
			return false;
	...
	protected void peaceMode()
	{
		...
		else
		{
			...
			// Random walk otherwise. (Exclude PK GUARDS FROM RETURN TO SPAWN WHILE CHASING A PLAYER)
			if ((npc.getNpcId() != 62326) && Config.RANDOM_WALK_RATE > 0 && !npc.isNoRndWalk() && Rnd.get(Config.RANDOM_WALK_RATE) == 0)
	...
	protected void combatMode()
	{
		...
		// If target is too far, stop hating the current target. Do nothing for this round. // ADD Guard Check
		if(npc.getNpcId()!=62326)
		{
		...
		{
			if (!npc.isIn3DRadius(target, 3200)) // Guard Check
			{
			npc.getAggroList().stopHate(target);
			return;
			}
		...
		if(npc.getNpcId() == 62326) // PK Killer Range
			range = 15000;
		...
		if (npc.getNpcId() != 62326 && npc.getTemplate().getAiType() == NpcAiType.ARCHER && dist <= (60 + combinedCollision) && Rnd.get(4) < 1)
		{

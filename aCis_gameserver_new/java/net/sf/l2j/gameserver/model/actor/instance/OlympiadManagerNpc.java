...
public class OlympiadManagerNpc extends Folk
{
	...
	public void showChatWindow(Player player, int val)
	{
		...
		switch (npcId)
		{
			case 31688: // Olympiad managers
				if (player.isNoble() && val == 0)
				{
					...
					filename= "blank"; //the custom one
					final NpcHtmlMessage html_noble = new NpcHtmlMessage(0);
					html_noble.setFile("data/html/olympiad/noble_main.htm");
					html_noble.replace("%oly_left%", Olympiad.getInstance().olympiadEnd_toString()); //return olympiad time left	
					html_noble.replace("%objectId%", getObjectId());
					player.sendPacket(html_noble);	
		...
		if (filename == "blank") // do not show extra htm window if player is noblesse (the custom one will apear)
			return;
		...
		if (command.startsWith("OlympiadNoble"))
		{
			...
			switch (val)
			{
				...
				case 4: // register non classed based
					...
					{
						if (item.getEnchantLevel() > 6 || (item.getItem().getCrystalType() == CrystalType.S))//Custom Code Oly +6 restriction
						{
							player.sendMessage("You can not register while holding an item higher than A grade or enchanted over than 6."); 
							return;
						}
					}

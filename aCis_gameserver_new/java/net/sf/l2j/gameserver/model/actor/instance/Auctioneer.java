...
public final class Auctioneer extends Folk
{
	...
	private void showAuctionsList(String val, Player player)
	{
		...
		//final int max = MathUtil.countPagesNumber(chs.size(), 16);
		
		// Cut auctions list up to page number.
		//chs = chs.subList((page - 1) * PAGE_LIMIT, Math.min(page * PAGE_LIMIT, chs.size()));
		chs = chs.subList((page - 1) * 16, Math.min(page * 16, chs.size()));
		...
		{
			final Auction auction = ch.getAuction();
			StringUtil.append(sb, "<tr><td>", ch.Auction_custom(ch.getId()), "</font></td><td width=8></td><td><font color=\"c45e39\"><a action=\"bypass -h npc_", getObjectId(), "_bidding ", ch.getId(), "\">", ch.getName(), " [", auction.getBidders().size(), "]</a></font></td><td>", sdf.format(auction.getEndDate()), "</td><td><font color=\"86bfbf\">", auction.getMinimumBid(), "</font></td></tr>");		
			//StringUtil.append(sb, "<tr><td><font color=\"aaaaff\">", ch.getLocation(), "</font></td><td width=8></td><td><font color=\"ffffaa\"><a action=\"bypass -h npc_", getObjectId(), "_bidding ", ch.getId(), "\">", ch.getName(), " [", auction.getBidders().size(), "]</a></font></td><td>", sdf.format(auction.getEndDate()), "</td><td><font color=\"aaffff\">", auction.getMinimumBid(), "</font></td></tr>");
		}

...
public class VillageMaster extends Folk
{
		...
		else if (command.startsWith("Subclass"))
		{
			...
			switch (cmdChoice)
			{
				...
				case 1: // Add Subclass - Initial
					...
					/*
					 * if (player.getStatus().isOverburden() || player.getWeightPenalty().ordinal() > 2) { player.sendPacket(SystemMessageId.NOT_SUBCLASS_WHILE_OVERWEIGHT); return; }
					 */
					
					// Avoid giving player an option to add a new sub class, if they have five already , default 3.
					if (player.getSubClasses().size() == 5)
					{
						html.setFile("data/html/villagemaster/SubClass_Fail.htm");
						player.sendMessage("You can not add more than 5 subclasses.");
						break;
					}
				...
				case 2: // Change Class - Initial
					...
					else
					{
						...
						StringUtil.append(sb, "<a action=\"bypass -h npc_%objectId%_Subclass 5 0\">", PlayerData.getInstance().getClassNameById(player.getBaseClass()), "<font color=93C47D>" + " (Base Class)</a></font><br>");
				...
				case 3: // Change/Cancel Subclass - Initial
					...
					if (player.getSubClasses().containsKey(4))
						html.replace("%sub4%", PlayerData.getInstance().getClassNameById(player.getSubClasses().get(4).getClassId()));
					else
						html.replace("<a action=\"bypass -h npc_%objectId%_Subclass 6 4\">%sub4%</a><br>", "");
					//
					// +1 class
					if (player.getSubClasses().containsKey(5))
						html.replace("%sub5%", PlayerData.getInstance().getClassNameById(player.getSubClasses().get(5).getClassId()));
					else
				...
				case 4: // Add Subclass - Action (Subclass 4 x[x])
					...
					{
						player.sendMessage("You should be over level 75 in order to add a new subclass.");
						allowAddition = false;
					}
					if (allowAddition)
					{
					...
					if (allowAddition && isValidNewSubClass(player, paramOne))
					{
						...
						{
							player.sendMessage("Maximum amount of subclasses have been reached.");
							return;
						}
						player.disarmWeapon(true);// disarm left and right handed weapons
						//player.getInventory().unequipItemInBodySlot(Item.SLOT_CHEST); // disarm player's chest
				...
				case 5: // Change Class - Action
					...
					/*
					 * if (paramOne == 0) { System.out.println("Error on Subclass "+ "ClassIndex() is "+ player.getClassIndex() + " paramOne is " +paramOne +"for player " + player.getName()); return; }
					 */
					/*
					 * if (paramOne == 0) { if (!checkVillageMaster(player.getBaseClass())) return; } else {
					 */
					/*
					 * try { if (!checkVillageMaster(player.getSubClasses().get(paramOne).getClassDefinition())) return; } catch (NullPointerException e) { return; }
					 */
					// }
					player.disarmWeapon(true);// disarm left and right handed weapons
					//player.getInventory().unequipItemInBodySlot(Item.SLOT_CHEST); // disarm player's chest
					player.getInventory().unequipItemInSlot(14); //disarm wings
	...
	private final static Set<ClassId> getAvailableSubClasses(Player player)
	{
		...
		if (availSubs != null && !availSubs.isEmpty())
		{
			for (Iterator<ClassId> availSub = availSubs.iterator(); availSub.hasNext();)
			{
				...
				/*
				 * // check for the village master // restriction All subs to all npcs if (!checkVillageMaster(classId)) { availSub.remove(); continue; }
				 */
	...
	{
		// check for the village master // restriction All subs to all npcs

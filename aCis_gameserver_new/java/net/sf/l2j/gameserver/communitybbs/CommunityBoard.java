...
import net.sf.l2j.gameserver.communitybbs.manager.ServerInfoBBSManager;
import net.sf.l2j.gameserver.communitybbs.manager.TopBBSManager;
import net.sf.l2j.gameserver.communitybbs.manager.TopClansBBSManager;
...
public class CommunityBoard
{
	...
	public void handleCommands(GameClient client, String command)
	{
		...
		else if(command.startsWith("_bbsgetfav") || (command.startsWith("_bbspvp")|| (command.startsWith("_bbspk")))) //old favorites (now top pvp / top pk )
		...
		else if (command.startsWith("_bbsloc") || (command.startsWith("_bbstopclan"))) // old Region (now top clans)
			TopClansBBSManager.getInstance().parseCmd(command, player);
			//RegionBBSManager.getInstance().parseCmd(command, player);
		else if (command.startsWith("_bbsclan")) // old clans (now grand bosses)
			BossBBSManager.getInstance().parseCmd(command, player);
			//ClanBBSManager.getInstance().parseCmd(command, player);
		else if (command.startsWith("_bbsmemo"))// old memo (now clan bosses)
			ClanBossBBSManager.getInstance().parseCmd(command, player);
		...
		//else if (command.startsWith("_bbspvp"))
			  //TopBBSManager.getInstance().parseCmd(command, player);
		else

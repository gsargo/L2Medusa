 ...
 * @author Anarchy
 */
public class VipCoin3 implements IItemHandler
{
	@Override
	public void useItem(Playable playable, ItemInstance item, boolean forceUse)
	{
		...
		{
			player.setVIP(true);
			player.setVIPUntil(System.currentTimeMillis() + (1000 * 60 * 60 * 24 * 24));
			player.store();
			player.sendMessage("You are now a premium member for 24 days.");
			...
			//final L2Skill Vitality = SkillTable.getInstance().getInfo(9919, 1); // Add Vitality Effect as a skill on golden premiums
			//player.addSkill(Vitality, true, true);
			final String VIP_SOUND = Quest.SOUND_FANFARE;
			Quest.playSound((player), VIP_SOUND);
